/// Darbitex Bridge v3 — AMM bridge for pegged assets, FungibleAsset.
///
/// Uses constant-product formula (x*y=k) with depeg protection via
/// MAX_IMBALANCE_PCT cap (80%). NOT a StableSwap curve — designed for
/// bridged asset pairs where simplicity + depeg guard is sufficient.
/// For true low-slippage stableswap, a Curve-style A-parameter module
/// would be needed as a future upgrade.
///
/// Audit fixes:
///   [B-C2] Proportional LP minting in add_bridge_liquidity
///   [B-M1] Unowned — pause via protocol admin only
///   [B-H3] Min fee of 1 unit for small amounts
///   [EXT-C2] Renamed from "Stableswap" — formula is CPMM, not StableSwap
...
module darbitex::bridge {
    ...
    use darbitex::pool; // for protocol_config admin check

    // ===== Constants =====
    const FEE_BPS: u64 = 1;
    const BPS_DENOM: u64 = 10_000;
    const MAX_IMBALANCE_PCT: u64 = 80;
    const MINIMUM_LIQUIDITY: u64 = 1000; // [CL-04] Min LP lock, same as pool
    ...
    const E_LOCKED: u64 = 10;          // [EXT2] Reentrancy guard

    // ===== Resources =====

    /// Bridge pool — UNOWNED (no owner field). [B-M1]
    struct BridgePool has key {
        ...
        total_bridged: u128,
        paused: bool,
        locked: bool,           // [EXT2] Reentrancy guard
    }
    ...
    public entry fun create_bridge(
        ...
        // [CL-04] Ensure initial liquidity is sufficient
        ...
        // [B-C1] Per-pool LP. [CL-04] Lock MINIMUM_LIQUIDITY, rest to creator.
    ...
    /// [B-C2] Proportional LP minting (not simple sum).
    public entry fun add_bridge_liquidity(
        ...
        // [B-C2] Proportional check (5% tolerance)
        let expected_to = ((amount_from as u128) * (pool.reserve_to as u128) / (pool.reserve_from as u128) as u64);
        // [H3] Use addition to avoid u64 underflow when expected_to < tolerance
    ...
    public entry fun remove_bridge_liquidity(
        ...
        // [CL-04] Guard: can't drain below MINIMUM_LIQUIDITY
