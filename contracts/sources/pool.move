/// Darbitex Pool v3 — AMM with permissionless V4 hooks, FungibleAsset.
///
...
///
/// Audit fixes applied: P-C1, P-C2, P-H1, P-H2, P-M1, P-M2, P-M4,
///   B-C1 (per-pool LP), LP-H2 (friend-only LP).
...
module darbitex::pool {
    ...
    struct Pool has key {
        ...
        // [P-H1][P-H2] Internal reserve tracking (not store balance)
        reserve_a: u64,
        reserve_b: u64,
        locked: bool,      // [P-M1] reentrancy guard (swap + flash)
        paused: bool,
        lp_supply: u64,
        total_swaps: u64,
        total_volume_a: u128,
    ...
    struct FlashReceipt {
        ...
        k_before_hi: u128,
        k_before_lo: u128,
        borrowed_a: bool,       // [C1] true = token A, false = token B
        borrow_amount: u64,     // [H2] amount borrowed
        store_before: u64,      // [H2] store balance before borrow (fee enforcement)
    }
    ...
    /// [P-C2] Strict greater than for flash loan fee enforcement
    ...
    fun assert_admin(account: &signer) acquires ProtocolConfig {
    ...
    public entry fun init_protocol(deployer: &signer, factory_addr: address) {
        ...
        move_to(deployer, ProtocolConfig { admin: addr, pending_admin: option::none(), treasury: addr, factory_addr });
    }

    /// [M1] 2-step admin transfer: propose → accept. Prevents lockout from typo.
    public entry fun propose_admin(admin: &signer, new_admin: address) acquires ProtocolConfig {
        assert_admin(admin);
        let c = borrow_global_mut<ProtocolConfig>(@darbitex);
        c.pending_admin = option::some(new_admin);
    }

    public entry fun accept_admin(new_admin: &signer) acquires ProtocolConfig {
        let c = borrow_global_mut<ProtocolConfig>(@darbitex);
        assert!(option::is_some(&c.pending_admin), E_NOT_ADMIN);
        let pending = *option::borrow(&c.pending_admin);
        assert!(signer::address_of(new_admin) == pending, E_NOT_ADMIN);
        c.admin = pending;
        c.pending_admin = option::none();
        event::emit(ProtocolConfigUpdated { admin: pending, treasury: c.treasury, factory_addr: c.factory_addr });
    }

    public entry fun set_treasury(admin: &signer, new_treasury: address) acquires ProtocolConfig {
        assert_admin(admin);
        let c = borrow_global_mut<ProtocolConfig>(@darbitex);
        c.treasury = new_treasury;
        event::emit(ProtocolConfigUpdated { admin: c.admin, treasury: new_treasury, factory_addr: c.factory_addr });
    }
    ...
    public fun create_pool(
        ...
        move_to(&pool_signer, Pool {
            ...
            reserve_a: amount_a,  // [P-H1] internal tracking
            reserve_b: amount_b,
        ...
        // LP to factory (factory transfers to creator)
    ...
    public entry fun withdraw_protocol_fee(
        ...
        assert_admin(admin);
        let treasury = borrow_global<ProtocolConfig>(@darbitex).treasury;
        let pool = borrow_global_mut<Pool>(pool_addr);
    ...
    /// [EXT-H1] Extracted TWAP update — called from swap + liquidity ops.
    ...
    /// [P-M1] Reentrancy guarded. [P-H1] Uses internal reserves.
    fun swap_internal(
        pool: &mut Pool,
        pool_addr: address,
        swapper: address,
        ...
        // [P-H1][P-H2] Use internal reserves, not store balance
        ...
        // Fee accounting. [CL-01] Min extra_fee of 1 to prevent zero-fee dust swaps.
        ...
        // [P-H1] Update internal reserves (exclude accumulated fees)
        if (a_to_b) {
        ...
        pool.locked = false; // [P-M1] unlock

        event::emit(Swapped {
            swapper, pool_addr, // [P-M4] actual swapper
    ...
    fun add_liquidity_internal(
        ...
        assert!(!pool.paused, E_PAUSED);
        assert!(!pool.locked, E_LOCKED); // [EXT-C1] Block during flash loan
        update_twap(pool); // [EXT-H1] Update TWAP before reserve change

        // [P-H1] Use internal reserves
        let expected_b = ((amount_a as u128) * (pool.reserve_b as u128) / (pool.reserve_a as u128) as u64);
        // [P-M2] Min tolerance of 1. [H3] Use addition to avoid u64 underflow.
        // [DS-01] Use u128 comparison to avoid u64 overflow on extreme ratios.
    ...
    /// Remove liquidity. NOTE: intentionally allowed during pause (users must
    /// be able to exit during emergency). Blocked during flash loan only.
    fun remove_liquidity_internal(
        ...
        let provider_addr = signer::address_of(provider);
        // [B-C1] Per-pool LP balance check
    ...
    // Plain pools: no flash loans. [P-C2] Strict k increase.
    ...
    public fun flash_borrow_hooked(
        ...
        // [C1] Validate metadata is one of pool's tokens — prevents stray token theft
        let borrowed_a = if (metadata == pool.metadata_a) { true }
        else { assert!(metadata == pool.metadata_b, E_WRONG_TOKEN); false };

        // [H2] Snapshot store balance before borrow (for fee enforcement in flash_repay)
    ...
    /// [P-C2] Strict k increase + [H2] same-token fee enforcement.
    public fun flash_repay(pool_addr: address, receipt: FlashReceipt) acquires Pool {
        ...
        // [H2] Fee enforcement: borrowed token must be repaid in same token + fee.
        // Prevents zero-fee flash swaps (borrow A, repay only B).
        ...
        // Sync reserves from store (flash loan may have changed balances)
        ...
        let (k_hi, k_lo) = mul_u128((pool.reserve_a as u128), (pool.reserve_b as u128));
        // [P-C2] Strict greater than — must overpay
    ...
    /// [H1] Force-remove unclaimed hook. Rescues stuck pools where hook_addr
    /// is invalid or hook module never claimed. Only works if hook NOT claimed.
    ...
    public entry fun pause(admin: &signer, pool_addr: address) acquires ProtocolConfig, Pool {
        assert_admin(admin);
        borrow_global_mut<Pool>(pool_addr).paused = true;
        event::emit(PoolPaused { pool_addr, paused: true });
    }

    public entry fun unpause(admin: &signer, pool_addr: address) acquires ProtocolConfig, Pool {
        assert_admin(admin);
    ...
    public fun reserves(pool_addr: address): (u64, u64) acquires Pool {
        ...
        (p.reserve_a, p.reserve_b) // [P-H1] internal reserves
    }
    ...
    public fun protocol_config(): (address, address, address) acquires ProtocolConfig {
        let c = borrow_global<ProtocolConfig>(@darbitex);
        (c.admin, c.treasury, c.factory_addr)
    }

    #[view]
    /// [L-2] Check if pool exists at address
