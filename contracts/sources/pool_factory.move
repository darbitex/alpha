/// Darbitex Pool Factory — Canonical pools + Hook Auction.
/// 1 pair = 1 pool. Hook via permanent auction. Resale supported.
///
/// Audit fixes: F-C1 (no-bidder guard), F-C2 (resale escrow),
///   F-H1 (CoinStore), F-H2 (re-check in finalize), F-M3 (max duration),
///   F-L1 (min bid).
...
module darbitex::pool_factory {
    ...
    const MAX_AUCTION_DURATION: u64 = 2592000;   // [F-M3] 30 days max
    const MIN_BID_INCREMENT: u64 = 10;           // 10%
    const MIN_INITIAL_BID: u64 = 10_000_000_000; // [F-L1] 100 APT
    const ANTI_SNIPE_WINDOW: u64 = 600;          // [EXT-M1] 10 min anti-snipe
    ...
    const E_NO_BIDDER: u64 = 14;       // [F-C1]
    const E_DURATION: u64 = 15;        // [F-M3]
    ...
    struct Auction has store, drop {
        ...
        has_bid: bool,              // [F-C1] track if any real bid exists
    }
    ...
    public entry fun init_factory(deployer: &signer) {
        ...
        // [F-H1] Register CoinStore for APT escrow
    ...
    public entry fun create_canonical_pool(
        ...
        // Transfer LP from factory to creator [B-C1] per-pool LP
    ...
    public entry fun start_auction(
        ...
        assert!(exists<Factory>(@darbitex), E_NOT_INIT);
        assert_sorted(metadata_a, metadata_b);
        assert!(bid_amount >= MIN_INITIAL_BID, E_BID_TOO_LOW); // [F-L1]
        assert!(duration >= MIN_AUCTION_DURATION && duration <= MAX_AUCTION_DURATION, E_DURATION); // [F-M3]
    ...
    public entry fun bid(
        ...
        // [F-C2] For resale first bid, min_price is the floor
        let min_bid = if (!auction.has_bid) {
            auction.bid_amount // min_price for resale
        } else {
            auction.bid_amount + auction.bid_amount * MIN_BID_INCREMENT / 100
        };
        assert!(bid_amount >= min_bid, E_BID_TOO_LOW);

        // Refund previous bidder only if there was a real bid
        // [F-C2] Don't refund @0x0
        if (auction.has_bid) {
            let factory_signer = account::create_signer_with_capability(&factory.signer_cap);
        ...
        // [EXT-M1] Anti-snipe: extend deadline if bid within last 10 min
    ...
    public entry fun finalize_auction(
        ...
        // [F-C1] Must have at least one real bid
        assert!(auction.has_bid, E_NO_BIDDER);

        let factory_signer = account::create_signer_with_capability(&factory.signer_cap);

        // [F-H2] Re-verify pool state before setting hook
        if (auction.is_resale) {
            pool::remove_hook(&factory_signer, pool_addr);
        } else {
            // Re-check no hook was set in the meantime
            let (current_hook, _) = pool::pool_hook(pool_addr);
            assert!(option::is_none(&current_hook), E_ALREADY_HOOKED);
        };
    ...
    public entry fun cancel_auction(
        ...
        assert!(timestamp::now_seconds() >= auction.end_time, E_AUCTION_NOT_ENDED);
        assert!(!auction.has_bid, E_AUCTION_ACTIVE); // only cancel if no bids
    ...
    /// List hook for resale. Called by hook module via HookCap.
    /// [F-C2] No APT escrowed at listing — only when first bid comes in.
    public fun list_for_resale(
        ...
        let end_time = timestamp::now_seconds() + duration;
        table::add(&mut factory.auctions, pool_addr, Auction {
            hook_addr: @0x0,     // bidders specify their hook
            bidder: @0x0,
            bid_amount: min_price, // floor price
            end_time,
            is_resale: true,
            seller: option::some(seller),
            has_bid: false,        // [F-C1] no bid yet — can't finalize
        });
    ...
    #[view]
    /// [L1] Paginated pool list for large registries.
